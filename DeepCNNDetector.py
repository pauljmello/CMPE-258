...
#vid = cv2.VideoCapture("") # Prerecorded Video
vid = cv2.VideoCapture(0)  # Live Capture of Webcam

framePercent = 0.8  # Moderately Helps Control Frame Rate

model = load_model("MNIST Detection/mnist_detector.h5")
model.compile(optimizer = "Adam", loss = 'categorical_crossentropy', metrics = ['accuracy'])

processedVid = cv2.VideoWriter("yolact processed video.avi", cv2.VideoWriter_fourcc(*'MJPG'), 10,
                               (int(vid.get(3) * framePercent) , int(vid.get(4) * framePercent)))
...
while True:
    ...
    #cv2.imshow("Mello Source Frame", srcFrame)
